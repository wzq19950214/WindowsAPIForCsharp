...
namespace GTR
{
    class MY
    {
        ...
        {
            int TalkTimes = 0;
            Application.Run(new RC2APP.SplashTip("喊话开始", System.Drawing.Color.Red, 2000, "宋体", 100));

            #region//-------------发送按键测试-----------
            //string sendtext = "我的咖啡123,欢迎来abc,全场8折";
            //char[] pwd = sendtext.ToCharArray();
            //for (int j = 0; j < pwd.Length; j++)
            //{
            //    if (Regex.IsMatch(pwd[j].ToString(), @"[\u4e00-\u9fa5]"))
            //    {
            //        SendKeys.SendWait(pwd[j].ToString());
            //    }
            //    else
            //        KeyMouse.SendKeys(pwd[j].ToString(), 60);
            //}
            //KeyMouse.SendKeys(TkDataList,100);
            //KeyMouse.MouseClick(803, 933, 1, 1, 1000);
            //EnterClic();
            ...
            //Pa = ImageTool.fPic(Talk_Data2.Properties.Resources.主界面3, 0, 0, 0, 0, 24);
            //Pa = ImageTool.fPic(Talk_Data2.Properties.Resources.主界面1, 0, 0, 0, 0, 24);
            //Pb = ImageTool.fPic(Talk_Data2.Properties.Resources.主界面2, 0, 0, 0, 0, 24);
            //if(Pa.X>0||Pb.Y>0)
            //{ }
            #endregion//--------------------------------3
        ...
        public int Talk(string city)
        {
            Point Pa = new Point(-1, -2);
            Point Pb = new Point(-1, -2);
            for(int i=0;i<2;i++)
            {
                //Pa = ImageTool.fPic(Talk_Data2.Properties.Resources.主界面1, 980, 548, 1298, 677, 24);
                //Pb = ImageTool.fPic(Talk_Data2.Properties.Resources.主界面2, 980, 548, 1298, 677, 24);
                //线上
                Pa = ImageTool.fPic(Talk_Data2.Properties.Resources.主界面, 980, 548, 1298, 677, 24);
                Pb = ImageTool.fPic(Talk_Data2.Properties.Resources.主界面3, 980, 548, 1298, 677, 24);
                if (Pa.X < 0 && Pb.X < 0)
                {
                    MessageBox.Show("找不到主界面", "异常信息", MessageBoxButtons.OKCancel,
MessageBoxIcon.Information, MessageBoxDefaultButton.Button1, MessageBoxOptions.DefaultDesktopOnly);
            ...
            KeyMouse.MouseClick(991, 554, 1, 1, 500);//1288,666
            //MyClick(1288, 666, 500);//点击频道界面
            KeyMouse.MouseClick(632, 312, 1, 1, 1000);//753, 376
            //MyClick(753, 376, 400);//点击搜索栏
            try{
                SendKeys.SendWait(city);
                //KeyMouse.SendKeys(city,200);
            //Clipboard.Clear();
            //if(Clipboard.GetData(DataFormats.Text)!=null)
            //    Clipboard.Clear();
            //Clipboard.SetDataObject(city);
            }
            catch(Exception w)
            {throw;}

            
            //MyClick(804, 450, 200);//点击配置关键字频道
            KeyMouse.MouseClick(561, 373, 1, 1, 1000);//804,450
            //MyClick(848, 785, 300);//点击加入频道
            KeyMouse.MouseClick(630, 651, 1, 1, 1000);//848,785
            for (int i = 0; i < 2; i++)
            {
                //Pa = ImageTool.fPic(Talk_Data2.Properties.Resources.主界面1, 980, 548, 1298, 677, 24);
                //Pb = ImageTool.fPic(Talk_Data2.Properties.Resources.主界面2, 980, 548, 1298, 677, 24);
...
MessageBoxIcon.Information, MessageBoxDefaultButton.Button1, MessageBoxOptions.DefaultDesktopOnly);
            ...
            //MyClick(803, 933, 300);
            //KeyMouse.MouseClick(714, 772, 1, 1, 1000);//803,933
            foreach(string TkDataList in TalkDataList)
            {
                KeyMouse.MouseClick(714, 772, 1, 1, 1000);//803,933
                //SendKeys.SendWait(TkDataList);
                KeyMouse.MouseClick(714, 772, 1, 1, 1000);//803,933
                //EnterClic();
                char[] pwd = TkDataList.ToCharArray();
                for (int j = 0; j < pwd.Length; j++)
                {
                    if (Regex.IsMatch(pwd[j].ToString(), @"[\u4e00-\u9fa5]"))
                    {
                ...
                KeyMouse.SendEnterKey();
                //SendKeys.SendWait("~");
                int interval = Convert.ToInt32(m_strinterval) * 1000;
                new Thread(() =>
                {
                    Application.Run(new RC2APP.SplashTip("喊话间隔" + m_strinterval+"秒", System.Drawing.Color.Red, 2000, "宋体", 100));
                }).Start();
        ...
        public bool ReadTalkDetail()
        {
            #region 一般性错误检查
            string TalkInfoPath = m_strProgPath + "\\DataTalkInfo.txt";
            if (!File.Exists(TalkInfoPath))
            {
                MessageBox.Show(TalkInfoPath + "不存在", "缺少文件", MessageBoxButtons.OKCancel,
MessageBoxIcon.Information, MessageBoxDefaultButton.Button1, MessageBoxOptions.DefaultDesktopOnly);
                WriteToFile(TalkInfoPath + "不存在");
                return false;
            }
            m_strOrderData = FileRW.ReadFile(TalkInfoPath);
            if (m_strOrderData == "")
            {
                MessageBox.Show(("==========> 配置内容为空 <==========\n"));
                WriteToFile("==========> 配置内容为空 <==========\n");
                return false;
            }
            #endregion

            m_strinterval = MyStr.FindStr(m_strOrderData, "<seconds>", "</seconds>");

            string strItem = MyStr.FindStr(m_strOrderData, "[City]", "[TalkData]");
            CityList = new List<string>(strItem.Split(new string[] { ",", "\r\n" }, StringSplitOptions.RemoveEmptyEntries));

            strItem = MyStr.FindStr(m_strOrderData, "[TalkData]", "[End]");
            TalkDataList = new List<string>(strItem.Split(new string[] { "\r\n" }, StringSplitOptions.RemoveEmptyEntries));
            ...
            foreach (string i in CityList)
            {
                ...
                //{
                //    WriteToFile(i + " 城市中含有英文符号");
                //    return false;
                //}
                WriteToFile(i);
            }
            foreach (string i in TalkDataList)
            {
                WriteToFile(i);
                char[] pwd = i.ToCharArray();
                for (int j = 0; j < pwd.Length; j++)
                {
                    if (char.IsControl(pwd[j]))
                    {
                        WriteToFile("含有不可见的转义字符" + pwd[j]);
                        return false;
                    }
                }
            }
        ...
        /// <param name="y"></param>
        /// <param name="time"></param>
        public void MyClick(int x, int y, int time)
        {
            Point pc = Point.Empty;
            //User32API.GetCursorPos(out pc);
            //User32API.MouseEvent(MouseEventType.MOUSEEVENTF_MOVE, x-pc.X, y-pc.Y, 0, 0);
            User32API.SetCursorPos(x, y);
            Sleep(500);
            User32API.MouseEvent(MouseEventType.MOUSEEVENTF_LEFTDOWN, 0, 0, 0, 0);
            Sleep(time);
        ...
        public void EnterClic()
         {
             //模拟F10按键
             keybd_event((byte)Keys.Enter,0,0,0); //按下F10
             keybd_event((byte)Keys.Enter,0,2,0); //按下后松开F10
         }
    }
}
