...
namespace GTR
{
    ...
    class Game
    {
        ...
        public static int StartProcess(string proName, string args)
        {
            ...
            Info.FileName = proName;
            Info.RedirectStandardOutput = true;
            Info.RedirectStandardInput = true;
            Info.UseShellExecute = false;//不使用系统外壳程序启动，重定向时此处必须设为false
            Info.RedirectStandardOutput = true; //重定向输出，而不是默认的显示在dos控制台上
            ...
            try
            {
                ...
                //p.StandardInput.WriteLine(args.Trim());
              
                p = Process.Start(Info);
                p.StandardInput.Flush();
                string output = p.StandardOutput.ReadLine();
                LOL.WriteToFile(output);
                //Console.ReadKey();
                //LOL.WriteToFile(output);
